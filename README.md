...
The mock is stateful for its basic save/read path: saves append samples, later matching reads return them with generated identities, profile-specific stored recording methods, and device provenance, and pagination operates over that in-memory storage. Omitted devices remain absent in every profile. The polling profile also omits explicit unknown-only devices, while retaining useful manufacturer or model details and normalizing a missing type to `unknown`; the observer profile retains manufacturer and model only. The mock is intentionally append-only: it does not emulate sync-version replacement or change-token history, and its delete methods are workflow stubs rather than a native ownership model, so they do not remove stored samples. `resetNitroHealthMock()` creates fresh storage and clears previously saved samples.
