...
namespace margelo::nitro::nitrohealth {
  ...
  public:
    ...
    explicit NativeHealthStatistics(double startTimeMs, double endTimeMs, std::optional<double> sum, std::optional<double> avg, std::optional<double> min, std::optional<double> max, std::optional<double> duration, std::optional<NativeDistanceScope> scope, std::optional<std::string> timeZone): startTimeMs(startTimeMs), endTimeMs(endTimeMs), sum(sum), avg(avg), min(min), max(max), duration(duration), scope(scope), timeZone(timeZone) {}
