...
namespace margelo::nitro::nitrohealth {
  ...
  public:
    ...
    explicit NativeDistanceSample(NativeHealthSampleIdentity identity, NativeHealthDataOrigin origin, std::optional<NativeHealthDeviceInfo> device, NativeHealthRecordingMethod recordingMethod, double startTimeMs, double endTimeMs, double distanceMeters, NativeDistanceScope scope): identity(identity), origin(origin), device(device), recordingMethod(recordingMethod), startTimeMs(startTimeMs), endTimeMs(endTimeMs), distanceMeters(distanceMeters), scope(scope) {}
