...
namespace margelo::nitro::nitrohealth {
  ...
  public:
    ...
    explicit NativeSleepSample(NativeHealthSampleIdentity identity, NativeHealthDataOrigin origin, std::optional<NativeHealthDeviceInfo> device, NativeHealthRecordingMethod recordingMethod, NativeSleepSampleKind kind, double startTimeMs, double endTimeMs, std::optional<std::string> stage, std::optional<NativeSleepStageData> stageData): identity(identity), origin(origin), device(device), recordingMethod(recordingMethod), kind(kind), startTimeMs(startTimeMs), endTimeMs(endTimeMs), stage(stage), stageData(stageData) {}
