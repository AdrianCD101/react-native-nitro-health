...
describe('NitroHealth changes contract', () => {
  ...
  it('maps VO2 max metadata on upserts', async () => {
    const timeMs = Date.parse('2026-01-01T09:00:00.000Z')
    mockNitroHealth.getChanges.mockResolvedValue({
      changes: [
        {
          ...
          vo2MaxSamples: [
            {
              ...nativeRecordMetadata('vo2-record'),
              timeMs,
              millilitersPerKilogramPerMinute: 42.5,
              androidMeasurementMethod: 'cooperTest',
            },
          ],
        },
      ],
    ...
    expect(result.changes[0]).toMatchObject({
      type: 'upsert',
      samples: [{ metadata: { android: { measurementMethod: 'cooper_test' } } }],
    })
  })

  it('maps body temperature metadata on upserts', async () => {
    const timeMs = Date.parse('2026-01-01T09:00:00.000Z')
    mockNitroHealth.getChanges.mockResolvedValue({
      changes: [
        {
          ...
          bodyTemperatureSamples: [
            {
              ...nativeRecordMetadata('bt-record'),
              timeMs,
              celsius: 36.6,
              iosSensorLocation: 'temporalArtery',
            },
          ],
        },
      ],
    ...
    expect(result.changes[0]).toMatchObject({
      type: 'upsert',
      samples: [{ metadata: { ios: { sensorLocation: 'temporal_artery' } } }],
    })
  })
